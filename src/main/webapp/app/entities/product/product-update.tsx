...
export const ProductUpdate = (props: RouteComponentProps<{ id: string }>) => {
  ...
  const bedenValues = Object.keys(Beden);
  const handleClose = () => {
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              />
              <ValidatedField label={translate('storeApp.product.beden')} id="product-beden" name="beden" data-cy="beden" type="select">
                {bedenValues.map(beden => (
                  <option value={beden} key={beden}>
                    {translate('storeApp.Beden.' + beden)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedBlobField
