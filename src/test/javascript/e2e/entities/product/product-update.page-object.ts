...
export default class ProductUpdatePage {
  ...
  bedenSelect: ElementFinder = element(by.css('select#product-beden'));
  ...
  async setBedenSelect(beden) {
    await this.bedenSelect.sendKeys(beden);
  }

  async getBedenSelect() {
    return this.bedenSelect.element(by.css('option:checked')).getText();
  }

  async bedenSelectLastOption() {
    await this.bedenSelect.all(by.tagName('option')).last().click();
  }
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.bedenSelectLastOption();
    await waitUntilDisplayed(this.saveButton);
